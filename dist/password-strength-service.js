...
var PasswordStrengthService = (function () {
  function PasswordStrengthService() {
    _classCallCheck(this, PasswordStrengthService);
  }

  _createClass(PasswordStrengthService, [{
    key: 'measureStrength',
    value: function measureStrength(p) {
      var stringReverse = function stringReverse(str) {
        for (var i = str.length - 1, out = ''; i >= 0;) {
          out += str[i--];
        }
        return out;
      },
      ...
      if (p) {
        ...
        tmp = _lodash2['default'].reduce(counts.pos, function (memo, val) {
        ...
        // repeated chars
        counts.neg.repeated = _lodash2['default'].chain(p.toLowerCase().split('')).countBy(function (val) {
          return val;
        }).reject(function (val) {
          return val === 1;
...
exports['default'] = PasswordStrengthService;
